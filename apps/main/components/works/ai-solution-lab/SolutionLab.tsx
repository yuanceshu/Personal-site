...
async function requestStage(payload: LabRequest, signal: AbortSignal) {
  const response = await fetch("/api/experiments/ai-solution-lab", {
    method: "POST",
  ...
  if (!response.ok)
    throw new Error(
      response.status === 503
        ? "当前尚未连接诊断服务。输入已保留，可以稍后重试，或返回选择一个示例体验。"
        : response.status === 429
          ? "操作有些频繁，请稍后重试。已有内容仍然保留。"
          : response.status === 422
            ? "当前选择与需求存在冲突，请检查产品方向、AI偏好与所选能力。"
            : failureMessage,
    );
  const result = responseSchema.safeParse(await response.json());
  if (!result.success || result.data.stage !== payload.stage)
            ...
            >
              ...
              {exampleId && (
                <p className="lab-note">
                  已选择预设示例，将使用演示诊断；修改描述后将尝试真实诊断服务。
                </p>
              )}
            </form>
