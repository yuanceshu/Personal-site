...
export function containsIdentity(value: string) {
  ...
  return /\d{7,}/.test(withoutDates.replace(/[\s+()（）-]/g, ""));
}
const text = z.string().trim().min(1).max(1200).refine((v) => !containsIdentity(v));
const city = z.string().trim().min(1).max(30).refine((v) => !/\d{7,}/.test(v));
...
export const interpretationSchema = z.strictObject({
  intent: z.enum(["search_trips", "clarify", "list_orders", "faq", "select_trip", "unsupported"]),
