    ...
    <div className={`${styles.worldPage} ${styles[article.theme]}`}>
      ...
      <header className={styles.worldHeader}>
        ...
        >
          ...
          <SiteMark className={styles.worldBackMark} tone="light" />
          静眠森
        </Link>
      </header>
        ...
        <article className={styles.articleBody}>
          ...
          {article.featuredStory ? (
            <Link
              ...
              href={article.featuredStory.href}
              aria-label={`阅读《${article.featuredStory.title}》`}
            >
              <picture className={styles.featuredStoryCover}>
                <source
                  srcSet={article.featuredStory.cover.avif}
                  type="image/avif"
                />
                <img
                  src={article.featuredStory.cover.webp}
                  alt={article.featuredStory.cover.alt}
                  loading="lazy"
                  decoding="async"
                  style={{
                    objectPosition: article.featuredStory.cover.position,
                  }}
                />
              </picture>
              <span className={styles.featuredStoryShade} aria-hidden="true" />
              <span className={styles.featuredStoryCopy}>
                <span>{article.featuredStory.eyebrow}</span>
                <strong>{article.featuredStory.title}</strong>
                <span>{article.featuredStory.description}</span>
                <small>
                  {article.featuredStory.meta}
                  <span aria-hidden="true"> →</span>
                </small>
              </span>
            </Link>
      ...
      <footer className={styles.worldFooter}>
        <Link className={styles.worldFooterBrand} href="/">
          <SiteMark tone="light" />
          <span>返回静眠森</span>
        </Link>
